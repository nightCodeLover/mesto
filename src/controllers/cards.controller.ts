...
export const createPostCardController = (client: MongoClient) => {
  return async (
    ...
    await createCard({ name, link, client, ownerId: mockOwner });

    res.send({});
  };
};

export const createPutCardLikesController = (client: MongoClient) => {
  return async (req: Request<{ cardId: string }>, res: Response) => {
...
export const createDeleteLikeFromCardController = (client: MongoClient) => {
  return async (req: Request<{ cardId: string }>, res: Response) => {
    const { cardId } = req.params;
...
export const createDeleteCardController = (client: MongoClient) => {
